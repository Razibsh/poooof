...
- 2026-07-05: **Keep the framework's MD files self-organizing (esp. BACKLOG.md)** — flat backlogs rot into an unreadable wall over time (Razi's live pain on Shyft / Airtable / desktop). Two parts: (1) a stronger template convention + agent rule so ideas stay dated, newest-first, and grouped by status (Inbox → Active/Promoted → Done) — and optionally by `[area]` — maintained automatically, with a "tidy pass" (standalone in a command and/or folded into `poooof:handoff`); (2) a one-time reorganizer that cleans up an *existing* messy backlog in a project already using the framework. Interacts with [[optional Linear mode]] — for projects tracking backlog in Linear, the markdown stays lean and Linear does the sorting; decide the split.
- 2026-07-05: Optional **Linear mode** for the framework — `/new-project` and `/adopt` ask whether to track backlog/tasks in Linear (official `mcp.linear.app` MCP, OAuth). Split truth model: Linear = idea/backlog capture + browsable/mobile view; ROADMAP.md/STATUS.md stay in-repo as the agent's execution plan. Template CLAUDE.md gains a Linear section (create issue on idea-capture, append issue ID to BACKLOG line, move issue In Progress/Done as work flows); `finish-stream` closes tracked issues on merge. No two-way sync, no hard dependency — remove the MCP + CLAUDE.md section and nothing breaks. Design *after* Razi's personal 2–4 week trial, from real usage notes.
